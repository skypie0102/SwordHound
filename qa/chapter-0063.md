...
Target 63 → E62 is verified by the complete sequence: shy messenger girl → Akwilla audience → Vikir formally freed/accepted into Ballak → Ah'Heman opposition → Akwilla offers mate/slave/material reward → Vikir rejects woman and hide → Aiyen signals him → request for Fountain of Valor → elder dispute → alarm bell / outsiders endpoint.
...
- PASS — Oxbear hide value logic is retained: cutting the hide in half would drastically reduce its value.
- PASS — **Fountain of Valor** remains the current working rendering of the Ballak sacred healing spring.
- PASS — spring access is restricted because the water is shallow, slow to replenish, and weakened by multiple simultaneous users.
- PASS — Ah'Heman/older traditionalists oppose Vikir's access; Aiyen argues that the Oxbear hunt proves his worth.
- PASS — Akwilla is personally inclined to grant the request but faces internal opposition.
- PASS — target 63 ends with the alarm: **“Outsiders have appeared!”**

## Boundary check

Target 64 switches cleanly to **Unfair Trade (1)**, and recovered English E63 contains the same outsider/Bourgeois-merchant opening.
