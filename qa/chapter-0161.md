# Chapter 161 QA — Familiar Face (4)

**Status:** PASS  
**Family acceptance:** contingent on `qa/families/familiar-face-0158-0161.md`

## Evidence
- Chinese: `source/chinese/chapters/161.txt` — `73a757520b40ef0438a9b3873511f190cd16588d`
- English witness: **E160** — `5c5e33eba33c4d9836193d6bbb29cca53770a817`
- Draft: `manuscript/drafts/chapter-0161.md` — `882eed710965f3871f18534377e9486641c289a3`
- Boundary: C162 — `680770fdf7b23729571a227210cf1346b0a82774` / E161 — `2cce07a6f2cc14c6860fee7a3ca615a3d70e001d` begin **Sin and Punishment (1)**.

## Checks
- PASS — Geronto remains a young female mage with red hair and extremely high mana output.
- PASS — Dolores estimates Geronto's magic at at least fifth-circle, possibly sixth-circle.
- PASS — Vikir's insistence on seeing Geronto's face drives him through the fire/iron-spike furnace; the motivation is identity verification, not recklessness without cause.
- PASS — Geronto's head detonates when the black face covering is removed, preventing direct facial identification.
- PASS — the corpse examination is clinical and limited to identity evidence; no erotic framing is added.
- PASS — Geronto is explicitly **not Camus Morgue**.
- PASS — Vikir's suspicion is source-supported by red hair, similar build, iron/fire magic, Dark Faction transfer, and closed-door training.
- PASS — the disproof uses body-size/age mismatch plus an old identifying mole below Camus's collarbone from a childhood training accident; no later Camus developments are imported.
- PASS — Geronto is still strongly implied to be a **Morgue** family corpse.
- PASS — Dolores's report of recent great-house grave robberies provides a plausible source-timed explanation for Baskerville/Donquixote/Quovadis/Morgue corpses becoming undead.
- PASS — all four black face coverings are recovered and together carry demonic power comparable in scale to Beelzebub, but their later exact provenance remains withheld.
- PASS — Ephebo, Hebe, Pedo, and Geronto are all confirmed eliminated by the end of the chapter.
- PASS — chapter ends exactly on **Quilt Rune Indulgentia** appearing above Vikir and Dolores, setting the boundary into *Sin and Punishment (1)*.


## Cycle-2 Phase 4 remediation

**Resolved 2026-09-26.** Restored the clinical identity-check detail: Geronto is a **naked, pale-skinned reconstructed female corpse** with an explicit anti-erotic clarification, and her **secondary physical development** is compared directly with Camus's.

The complete Familiar Face family was reread after remediation. Shifted mapping remains **158→E157, 159→E158, 160→E159, 161→E160**; target 162/E161 begins *Sin and Punishment (1)*.

## Phase-4 decision

**PASS at chapter level after remediation.** Final family acceptance is rebound through the refreshed Familiar Face family QA.
