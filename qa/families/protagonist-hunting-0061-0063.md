# Title-Family QA — The Protagonist of Hunting (1)–(3)

**Family status:** PASS — KINSHIP REBOUND  
**Target range:** Chapters 61–63  
**Following family:** target Chapter 64 begins *Unfair Trade (1)*.

## Evidence / mapping

| Target | Chinese source | Chinese SHA | Recovered-English witness | English SHA | Draft SHA | QA SHA |
|---|---|---|---|---|---|---|
| 61 | `061.txt` | `834abb36714ce9d7329e7854720ea20170ce5586` | **E60** | `cf367731f723c719b61b06070cb648fa9cf23a95` | `12578e5de23cc4c4eb0f279ad14ee20ad1cd5c8b` | `ec787604f0abda5c3bb072f0fc89ab3e1a62c016` |
| 62 | `062.txt` | `68c0110eec864429aebd42142991ba3694d8130f` | **E61** | `c03318d7153f27b179a5d2d7c36f4fa1e78d9dc2` | `aa267163bf0959623b7adc94f906518e8ac73ada` | `96233c67df63ff633095dc751b8a56f33d01ff41` |
| 63 | `063.txt` | `9db84e3df9443b7dbe9591327707cf2ff8677504` | **E62** | `72c9b67ea93f772a9ab6adadfce10f33f7de34ad` | `3a95a006e7bb54d8f35202366da98cff0cebe7a4` | `974cd89773078dda486907bb4cf5aa72d1b2cbcb` |
| boundary | `064.txt` — *Unfair Trade (1)* | `332ef149eb32e1c0ba4aa67a53410d8d310f5844` | **E63** — *Unfair Trade (1)* | `54c7cd980ebb6bc505aab824a8e5a26bbe40edfd` | next family | next family |

The one-chapter recovered-English lag established at the end of the previous family continues cleanly: **61→E60, 62→E61, 63→E62, 64→E63**.

## Title normalization

Chinese title wording varies:
- 61 — `丰收节的主角 (1)`
- 62 — `丰收祭的主角 (2)`
- 63 — `捕获祭典的主角 (3)`

Recovered English consistently uses **The Protagonist of Hunting (1)–(3)**. This is retained as the normalized production family title.

## Family continuity

### 60 → 61

Target 60 ends immediately after the male Oxbear kill. Target 61 opens on the Ballak harvest feast built around that carcass and converts the hunt into food, honor, Beelzebub growth, and social prestige.

### 61 → 62

Target 61 ends with Aiyen feeding Vikir a culturally suggestive but unnamed Oxbear organ under Ballak body-part folk belief. Target 62 opens after the festival has ended, with the tribe setting aside roughly 100 kg of premium meat as an offering to Madam Eight-Legs.

### 62 → 63

Target 62 ends after the village women compete over the soon-to-be-freed Vikir and Aiyen tells him she has already chosen what reward he should request. Target 63 begins with Akwilla summoning him, formally freeing him, and presenting the reward choice.

### 63 → 64

Target 63 ends as a sentry sounds the alarm that outsiders have arrived. Target 64 opens on the same alarm and reveals the outsiders as Imperial Bourgeois-family merchants, creating a clean family boundary.

## Cross-chapter decisions

- **Beelzebub active slots after the Oxbear kill:**
  1. **Incinerate — Cerberus (A+)**
  2. **Kilogram Hammer — Oxbear (A)**
  3. **Tough Life — Infernal Buffalo (A)**
- The source explicitly says the stronger Oxbear soul replaces the Hellhound soul. **Hemorrhage — Hellhound (B+)** is therefore displaced from the active three slots here.
- **Kilogram Hammer** follows the current indexed Fandom equipment terminology; recovered-English `Sacral Spine` and other variants are rejected.
- Ballak's inverse meat/honor custom is preserved: greater hunt contribution means a smaller meat portion but greater honor.
- Ballak body-part folk medicine is treated as tribal belief, not medical fact.
- The unnamed Oxbear organ in target 61 remains unnamed. The chapter's teasing implication is preserved without adding explicit sexual detail.
- **Madam Eight-Legs** receives a roughly 100 kg meat offering; the untouched offering and later white slime/dead vegetation establish that something claimed it.
- **Piranha Natteri** is retained for the target-62 monster window; Chinese controls its blue-green/red coloration over recovered-English color corruption.
- The line of village women is tied to Vikir's expected freedom and mate assignment. The pressure is preserved as social/reproductive context rather than romanticized.
- **Akwilla / Night Fox**, **Divine Archer Adonai**, **Ah'Heman**, **Aiyen**, and **Ahun** follow accepted/canonical English forms.
- Target 63's isolated **son** wording is superseded by direct continuity evidence inside the current source sequence: target 65 identifies **Aheul as Ahun's sister**, target 67 explicitly identifies **Aheul as Ah'Heman's granddaughter**, and indexed Fandom independently records Ah'Heman as grandfather of both. Target 63 therefore uses **grandson** as a scoped consistency correction.
- This kinship rebound does not import later Ah'Heman plot spoilers; only the already-present family relation is corrected.
- No later Ah'Heman conspiracy, origin, Red Death, necromancy, or death spoilers are imported.
- **Fountain of Valor** remains the working rendering for Ballak's sacred healing spring, which is shallow, slow to refill, and weakened by multiple simultaneous users.

## Explicitness / interpretation check

PASS. Reproductive and mate-assignment material remains source-faithful but non-erotic. No implicit organ is named beyond what the source states. No later relationship outcomes are imported.

## Family acceptance result

**PASS.** Targets 61–63 form one complete family and are ready for provenance/acceptance binding.
